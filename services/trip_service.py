...
async def get_trips(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Trip]:
    result = await db.execute(select(Trip).offset(skip).limit(limit).order_by(Trip.created_at.desc()))
    return list(result.scalars().all())


async def update_trip(db: AsyncSession, trip_id: str, data: TripUpdate) -> Optional[Trip]:
