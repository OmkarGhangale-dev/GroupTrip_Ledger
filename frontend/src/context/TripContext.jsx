...
export function TripProvider({ children }) {
  ...
  const loadTrips = useCallback(async (selectTripId = null) => {
    try {
      setLoading(true);
      setError("");
      ...
      const msg = err?.response?.data?.detail || "Could not connect to the backend server.";
      setError(msg);
      showToast(msg, "error");
    } finally {
      setLoading(false);
  ...
  const loadTripData = useCallback(async (tripId) => {
    ...
    try {
      ...
      const [
        ...
        getParticipants(tripId).catch(() => []),
        getExpenses(tripId).catch(() => []),
        getBookings(tripId).catch(() => []),
        getPayments(tripId).catch(() => []),
        getBalances(tripId).catch(() => []),
        getSettlements(tripId).catch(() => []),
      ...
      const msg = err?.response?.data?.detail || "Failed to load trip details.";
      setError(msg);
      showToast(msg, "error");
    } finally {
      setLoading(false);
  ...
  const editParticipant = async (participantId, participantData) => {
    try {
      const updated = await apiUpdateParticipant(participantId, participantData);
      showToast(`Updated ${updated.name}!`);
      await loadTripData(trip.id);
      return updated;
    } catch (err) {
      console.error("Failed to update participant:", err);
      const msg = err?.response?.data?.detail || "Failed to update participant.";
      showToast(msg, "error");
      throw err;
    }
  };
  ...
  const removeParticipant = async (participantId) => {
    try {
      ...
      const msg = err?.response?.data?.detail || "Failed to remove participant.";
      showToast(msg, "error");
      throw err;
    }
  };
  ...
  const editItineraryItem = async (itemId, itemData) => {
    try {
      ...
      const msg = err?.response?.data?.detail || "Failed to update itinerary item.";
      showToast(msg, "error");
      throw err;
    }
  };
  ...
  const removeItineraryItem = async (itemId) => {
    try {
      ...
      const msg = err?.response?.data?.detail || "Failed to delete itinerary item.";
      showToast(msg, "error");
      throw err;
    }
  };
  ...
  const totalPaymentsAmount = payments.reduce(
    (total, p) => (p.status === "completed" ? total + Number(p.amount || 0) : total),
    0
  );
