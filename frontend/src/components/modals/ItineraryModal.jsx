...
const ITEM_TYPES = [
  ...
  { id: "hotel", label: "🏨 Check-in / Stay" },
  { id: "flight", label: "✈️ Flight Departure / Arrival" },
  { id: "transport", label: "🚆 / 🚗 Travel Transit" },
  { id: "other", label: "📌 Other" },
];

export default function ItineraryModal({ isOpen, onClose, initialData = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMsg(err?.response?.data?.detail || "Failed to save itinerary item.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h3>{initialData ? "Edit Itinerary Event" : "Add Itinerary Event"}</h3>
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="modal-actions">
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? "Saving..." : initialData ? "Update Event" : "Add to Itinerary"}
            </button>
          </div>
        </form>
      </div>
