...
async def create_expense(data: ExpenseCreate, db: AsyncSession = Depends(get_db)):
    return await svc.create_expense(db, data)


@router.get("/trip/{trip_id}", response_model=List[ExpenseRead])
async def list_expenses(trip_id: str, db: AsyncSession = Depends(get_db)):
    return await svc.get_expenses_by_trip(db, trip_id)


@router.get("/{expense_id}", response_model=ExpenseRead)
async def get_expense(expense_id: str, db: AsyncSession = Depends(get_db)):
    e = await svc.get_expense(db, expense_id)
    if not e:
...
async def update_expense(expense_id: str, data: ExpenseUpdate, db: AsyncSession = Depends(get_db)):
    e = await svc.update_expense(db, expense_id, data)
    if not e:
        raise HTTPException(status_code=404, detail="Expense not found")
    return e
...
async def delete_expense(expense_id: str, db: AsyncSession = Depends(get_db)):
    deleted = await svc.delete_expense(db, expense_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Expense not found")
